...
-- 8.4. Interface languages -------------------------------------------
-- Real CRUD for the author to manage which languages exist — the actual
-- UI translation dictionary is a separate, much larger project this
-- doesn't include yet. Picking a non-default language here is currently
-- cosmetic (stored as a preference) until that translation layer exists.
...
insert into public.languages (code, name) values ('kk', 'Қазақша')
on conflict (code) do nothing;

-- Translation dictionary: key → per-language text. The author manages
-- this at /avtor/tilder/[code]. The app reads it by language code and
-- key. A missing translation falls back to the Kazakh (kk) value.
...
begin
  ...
  if v_book.status = 'published' then
    ...
    select s.user_id, 'new_chapter',
           ...
           '/kitapkhana/' || v_book.slug || '/' || new.chapter_number
    from public.shelf s
...
-- languages: everyone reads active ones; writer/admin can manage the list.
...
drop policy if exists "writer can manage languages" on public.languages;
create policy "writer can manage languages"
  on public.languages for all using (
    exists (select 1 from public.profiles p where p.id = auth.uid() and (p.is_writer or p.is_admin))
  );

-- translations: publicly readable for the app to render text; only
-- writer/admin can write.
...
drop policy if exists "writer can manage translations" on public.translations;
create policy "writer can manage translations"
