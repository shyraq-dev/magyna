...
type Settings = { font: FontKey; size: number; lineHeight: number; theme: ThemeKey; mode: PageMode };

const DEFAULTS: Settings = { font: "serif", size: 18, lineHeight: 1.9, theme: "light", mode: "scroll" };
const STORAGE_KEY = "magyna:reader-settings";
const PARAGRAPHS_PER_PAGE = 5;

/** Splits Markdown into "pages" for the paged reading mode — a simple
 *  paragraph-count heuristic (not literal fixed-height typesetting like
 *  a real e-reader, but delivers the actual paging *interaction* asked
 *  for: discrete pages with Prev/Next instead of one long scroll). */
function paginate(markdown: string): string[] {
  const blocks = markdown.split(/\n\s*\n/).filter((b) => b.trim());
  const pages: string[] = [];
  for (let i = 0; i < blocks.length; i += PARAGRAPHS_PER_PAGE) {
    pages.push(blocks.slice(i, i + PARAGRAPHS_PER_PAGE).join("\n\n"));
  }
    ...
    <div style={wrapperStyle}>
      ...
      {panelOpen && (
        ...
        >
          ...
          <div>
            <p className="mb-2 opacity-70">Парақтау форматы</p>
            <div className="flex gap-2">
              <button
                type="button"
                onClick={() => setSettings((s) => ({ ...s, mode: "scroll" }))}
                className="rounded-sm border px-3 py-1.5"
                style={{ borderColor: settings.mode === "scroll" ? theme.accent : theme.border }}
              >
                Үздіксіз
              </button>
              <button
                type="button"
                onClick={() => {
                  setSettings((s) => ({ ...s, mode: "paged" }));
                  setPage(0);
                }}
                className="rounded-sm border px-3 py-1.5"
                style={{ borderColor: settings.mode === "paged" ? theme.accent : theme.border }}
              >
                Парақтап
              </button>
            </div>
      ...
      {settings.mode === "scroll" ? (
        ...
        <div>
          <MarkdownContent content={pages[page]} />
          <div className="mt-8 flex items-center justify-between border-t pt-4" style={{ borderColor: theme.border }}>
            <button
              type="button"
              onClick={() => setPage((p) => Math.max(p - 1, 0))}
              disabled={page === 0}
              className="rounded-sm border px-4 py-2 text-sm disabled:opacity-30"
              style={{ borderColor: theme.border }}
            >
              ← Алдыңғы
            </button>
