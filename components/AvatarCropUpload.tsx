...
export default function AvatarCropUpload({
   ...
   *  avatar-preview-plus-button idle state. When set, the crop step
   *  renders as a full-screen modal instead of inline — otherwise it
   *  would distort whatever flex row the compact trigger sits in. */
  ...
  const cropPanel = (
    <div>
      ...
      <div className="mt-3 flex justify-center gap-3">
        <button type="button" onClick={() => setEditingSrc(null)} className="btn-secondary text-sm">
          Бас тарту
        </button>
        <button
          type="button"
  ...
  return (
    <div>
      {editingSrc ? (
        trigger ? (
          // Compact/embedded usage (e.g. the profile action row) — crop
          // as a full-screen modal so it can't distort the surrounding
          // flex layout.
          <div
            role="dialog"
            aria-modal="true"
          ...
          >
            <div className="w-full max-w-sm rounded-sm bg-paper p-6">{cropPanel}</div>
          </div>
        ) : (
          cropPanel
        )
