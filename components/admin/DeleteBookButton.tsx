...
export default function DeleteBookButton({ bookId }: { bookId: string }) {
  ...
  async function remove() {
    const ok = window.confirm(
      "Бұл кітапты толығымен жою керек пе? Барлық тараулар, пікірлер және сөредегі жазбалар да жойылады. Бұл әрекетті қайтару мүмкін емес."
    );
    if (!ok) return;
