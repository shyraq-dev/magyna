...
export default async function ProfilPage() {
  ...
  const { data: profile } = await supabase
    .from("profiles")
    .select(
      "username, full_name, avatar_url, wallpaper_url, is_admin, is_writer, telegram_id, bio, instagram_url, tiktok_url, website_url, telegram_link"
    )
    .eq("id", user.id)
    .single();
  ...
  return (
    <div className="mx-auto max-w-lg px-6 py-10">
      <div className="-mx-6 sm:-mx-24 lg:-mx-32 h-36 w-full overflow-hidden rounded-sm bg-nib-gradient sm:h-48">
        {profile?.wallpaper_url && (
          // eslint-disable-next-line @next/next/no-img-element
          <img src={profile.wallpaper_url} alt="" className="h-full w-full object-cover" />
        )}
      </div>

      <div className="-mt-8 flex items-end gap-5 px-2">
        <div className="rounded-full ring-4 ring-paper">
          <AvatarLightbox avatarUrl={profile?.avatar_url ?? null} initial={initial} />
        </div>
      </div>
      ...
      {/* Action row — horizontal (icon on top, label below each). */}
      <div className="mt-6 flex justify-center gap-8 border-y border-line py-4">
        <QuickAvatarUpload userId={user.id} currentAvatarUrl={profile?.avatar_url ?? null} />
        <Link href="/beyin/ondeu" className="flex flex-col items-center gap-1 text-xs text-muted transition hover:text-gold-600">
        ...
        <Link href="/baptaular" className="flex flex-col items-center gap-1 text-xs text-muted transition hover:text-gold-600">
          <span className="flex h-10 w-10 items-center justify-center rounded-full border border-line">
